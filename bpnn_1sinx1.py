...
class NN:
    ...
    def test(self, patterns):
        FOutput = open('1sinx1_Output.txt', 'w')
        for p in patterns:
    ...
    def train(self, patterns, iterations=300000, N=0.0006, M=0.0006):
        # N: learning rate
        # M: momentum factor
        # change = hidden_deltas[j]*self.ai[i]
        # self.wi[i][j] = self.wi[i][j] + N*change + M*self.ci[i][j]
        ...
        XLErrorOutput = open('1sinx1-XLError.txt', 'w')
...
def demo():
    ...
    n = NN(datalen, datalen+5, 1)

    n.train(traindata[0:TestFileXL[FileNum]], 300000, 0.01, 0.01) #def train(self, patterns, iterations=500, N=0.02, M=0.01):
...
# file_t = fopen('D:\!zju\!IntelligentSystem\HW#4\1sinx1_InputData.txt','r');
...
# plot(t,yt);
# hold;
# plot(xx,yy,'r');title('hide=6 iterations= N= M=');